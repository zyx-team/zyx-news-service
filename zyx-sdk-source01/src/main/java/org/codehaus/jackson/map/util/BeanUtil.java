...
public class BeanUtil
{
    ...
    */
     ...
     * result type is "org.springframework.cglib.proxy.Callback[]"
     *<p>
    ...
    protected static boolean isCglibGetCallbacks(AnnotatedMethod am)
    {
        ...
        /* And that type needs to be "org.springframework.cglib.proxy.Callback".
         * Theoretically could just be a type that implements it, but
         * for now let's keep things simple, fix if need be.
         */
        Class<?> compType = rt.getComponentType();
        // Actually, let's just verify it's a "org.springframework.cglib.*" class/interface
        Package pkg = compType.getPackage();
        if (pkg != null) {
            String pname = pkg.getName();
            if (pname.startsWith("org.springframework.cglib")
